...
- **修复嵌入式 Python 启动报错**：启动时自动将项目根目录加入 `sys.path`，解决 `ModuleNotFoundError: No module named 'src'`。
- **优化导航等待时间**：URL 稳定检测上限从 5s 降至 3s，导航后页面稳定等待从 2s 降至 1s，整体启动更快。
- **登录成功后延迟网络检测**：点击登录按钮后等待 2s 再做网络检测兜底，给页面足够时间响应登录请求。
- **监控循环增加缓冲**：登录任务完成后等待 2s 再进入下一次网络检测，避免过于频繁的检测。
- **页面弹窗拦截与日志记录**：自动捕获校园网页面的 alert/confirm 弹窗，延迟 1.5s 关闭让用户可见，同时记录弹窗内容到日志。
