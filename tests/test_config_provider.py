...
class TestGetRuntimeConfigReturnsCopy:
    ...
    def test_modify_result_does_not_affect_cache(self, provider: RuntimeConfigProvider) -> None:
        # 首先加载配置
        with patch("app.services.config_provider.load_ui_config") as mock_ui, \
    ...
    def test_two_calls_return_different_objects(self, provider: RuntimeConfigProvider) -> None:
        with patch("app.services.config_provider.load_ui_config") as mock_ui, \
             patch("app.services.config_provider.load_runtime_config") as mock_rt, \
             patch("app.services.config_provider.build_runtime_config") as mock_build:
            mock_ui.return_value = MonitorConfigPayload()
...
class TestGetUiConfigReturnsCopy:
    ...
    def test_returns_monitor_config_payload(self, provider: RuntimeConfigProvider) -> None:
...
class TestReload:
    ...
    def test_reload_updates_runtime_config(self, provider: RuntimeConfigProvider) -> None:
        with patch("app.services.config_provider.load_ui_config"), \
             patch("app.services.config_provider.load_runtime_config") as mock_rt, \
             patch("app.services.config_provider.build_runtime_config") as mock_build:
            mock_rt.return_value = (MonitorConfigPayload(), False)
    ...
    def test_reload_logs_warning_on_decrypt_error(self, provider: RuntimeConfigProvider) -> None:
        with patch("app.services.config_provider.load_ui_config"), \
             patch("app.services.config_provider.load_runtime_config") as mock_rt, \
             patch("app.services.config_provider.build_runtime_config"), \
...
class TestInitialValues:
    ...
    def test_runtime_config_initially_empty(self, provider: RuntimeConfigProvider) -> None:
