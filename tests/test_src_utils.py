...
from src.utils.browser import BrowserContextManager, STEALTH_INIT_SCRIPT
from src.utils.exceptions import LoginCancelledError
from src.system_tray import SystemTray
from src.playwright_bootstrap import (
    _candidate_hosts,
    _is_enabled,
    _has_chromium,
...
class TestSendNotification:
    @patch("src.utils.notify.is_windows", return_value=True)
    @patch("src.utils.notify._notify_windows", return_value=True)
    ...
    @patch("src.utils.notify.is_windows", return_value=False)
    @patch("src.utils.notify.is_macos", return_value=True)
    @patch("src.utils.notify._notify_macos", return_value=True)
    def test_macos(self, mock_mac, mock_is_mac, mock_is_win):
        assert send_notification("标题", "消息") is True
        mock_mac.assert_called_once_with("标题", "消息")

    @patch("src.utils.notify.is_windows", return_value=False)
    @patch("src.utils.notify.is_macos", return_value=False)
    @patch("src.utils.notify.is_linux", return_value=True)
    @patch("src.utils.notify._notify_linux", return_value=True)
    ...
    @patch("src.utils.notify.is_windows", return_value=False)
    @patch("src.utils.notify.is_macos", return_value=False)
    @patch("src.utils.notify.is_linux", return_value=False)
    def test_unsupported_platform(self, mock_is_linux, mock_is_mac, mock_is_win):
        assert send_notification("标题", "消息") is False

    @patch("src.utils.notify.is_windows", return_value=True)
    @patch("src.utils.notify._notify_windows", side_effect=Exception("fail"))
    ...
    @patch("src.utils.notify.is_windows", return_value=True)
    @patch("src.utils.notify._notify_windows", return_value=False)
...
class TestNotifyLinux:
    @patch("src.utils.notify.shutil.which", return_value="/usr/bin/notify-send")
    @patch("src.utils.notify.subprocess.run")
    ...
    @patch("src.utils.notify.shutil.which", return_value=None)
...
class TestSystemTrayMethods:
    ...
    @patch("src.system_tray.pystray.Icon")
...
class TestHasChromium:
    @patch("src.playwright_bootstrap.is_windows", return_value=True)
    def test_windows_no_cache_dir(self, mock_is_win):
        with patch("src.playwright_bootstrap.Path.home", return_value=Path("/nonexistent")):
            with patch("importlib.util.find_spec", return_value=None):
                with patch.dict("sys.modules", {"playwright.sync_api": None, "playwright": None}):
                    assert _has_chromium() is False

    @patch("src.playwright_bootstrap.is_windows", return_value=False)
    @patch("src.playwright_bootstrap.is_macos", return_value=False)
...
class TestEnsurePlaywrightReady:
    ...
    @patch("src.playwright_bootstrap._has_chromium", return_value=True)
    ...
    @patch("src.playwright_bootstrap._has_chromium", return_value=False)
    @patch("src.playwright_bootstrap._is_enabled", return_value=True)
...
class TestCleanupOrphanBrowsers:
    @patch("src.playwright_worker.sys")
    @patch("src.playwright_worker._cleanup_windows")
    ...
    @patch("src.playwright_worker.sys")
    @patch("src.playwright_worker._cleanup_posix")
