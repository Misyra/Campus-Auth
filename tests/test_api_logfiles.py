...
class TestParseLogLine:
    ...
    def test_warning_level(self):
        """WARNING 级别解析。"""
        raw = "[2026-06-01 12:00:00][WARNING][back][service] 警告"
        line = _parse_log_line(raw)
    ...
    def test_multiline_message(self):
        """消息中包含特殊字符。"""
        raw = "[2026-06-01 00:00:00][INFO][back][mod] key=value, a=b"
        line = _parse_log_line(raw)
...
class TestGetLogFileContent:
    ...
    def test_basic_content(self, tmp_path):
        """基本内容读取。"""
        content = "[2026-06-01 00:00:00][INFO][back][mod] hello\n"
    ...
    def test_level_filter(self, tmp_path):
        """级别过滤。"""
        content = (
            "[2026-06-01 00:00:00][INFO][back][mod] info msg\n"
            "[2026-06-01 00:00:01][ERROR][back][mod] error msg\n"
            "[2026-06-01 00:00:02][INFO][back][mod] info msg 2\n"
        )
        self._create_log_file(tmp_path, "2026-06-01", "app.log", content)
    ...
    def test_search_filter(self, tmp_path):
        """关键词搜索。"""
        content = (
            "[2026-06-01 00:00:00][INFO][back][mod] 登录成功\n"
            "[2026-06-01 00:00:01][ERROR][back][mod] 连接超时\n"
            "[2026-06-01 00:00:02][INFO][back][mod] 网络正常\n"
        )
        self._create_log_file(tmp_path, "2026-06-01", "app.log", content)
    ...
    def test_search_case_insensitive(self, tmp_path):
        """搜索大小写不敏感。"""
        content = "[2026-06-01 00:00:00][INFO][back][mod] Hello World\n"
        self._create_log_file(tmp_path, "2026-06-01", "app.log", content)
    ...
    def test_limit_applied(self, tmp_path):
        """限制返回行数。"""
        lines = [
            f"[2026-06-01 00:00:{i:02d}][INFO][back][mod] msg {i}\n"
            for i in range(100)
        ]
        content = "".join(lines)
        self._create_log_file(tmp_path, "2026-06-01", "app.log", content)
