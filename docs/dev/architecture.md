...
| 5（最高） | 自定义变量 | `config["custom_variables"]`（过滤掉系统保留变量名如 `PATH`、`HOME` 等） |
...
系统统一使用网络连通性检测判断任务成功与否：任务步骤全部完成后，自动检测网络是否可达（超时约 10 秒）。网络通 = 认证成功，网络断 = 认证失败。检测失败时会在日志中显示具体原因。原有 `success_conditions` 字段仅保留兼容性，不再参与判断。
