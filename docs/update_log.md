...
- **轻量模式托盘支持**：轻量模式新增独立的托盘开关（`lightweight_tray`），点击托盘中的"打开控制台"可按需唤醒 Web 管理界面
...
- 修复 `--tray/--no-tray` 参数未同时控制轻量模式托盘的问题
- 修复轻量模式按需启动 Web 服务时事件循环不匹配导致的崩溃
- 修复 `NullWebSocketManager` 未接受 WebSocket 连接导致的异常
