...
6. **Proxy 必须完整实现 PlaywrightWorker 公共接口**：`shutdown_worker()` 调用 `_worker.stop()`，`get_worker()` 调用 `_worker.is_alive()` 判断重启。Proxy 不是简单替换，必须完整实现：
   - `start()`：Popen 启动子进程，等待 ready 事件
   - `stop(timeout)`：写 CMD_SHUTDOWN → wait SHUTDOWN_GRACE → kill → join
   - `is_alive()`：子进程 `poll() is None`
   - `submit(cmd_type, data, wait, timeout) -> WorkerResponse`：分配 id → 写 stdin → 阻塞等响应（wait=False 时只写不等，但当前所有调用方都用 wait=True，保留参数仅为接口兼容）
...
| `app/workers/worker_proc.py` | 子进程入口 `main()`：设置 `CAMPUS_AUTH_ROLE=worker` → import 业务模块 → 调用 `cleanup_orphan_browsers()` → stdin 读循环 → `_dispatch` → stdout 写响应；`python -m app.workers.worker_proc` 可独立运行调试。模块体不 import 业务模块，避免角色判断时机问题 |

### 7.2 修改文件（9 个）

| 文件 | 改动 |
|---|---|
| `app/workers/playwright_worker.py` | 保留 `PlaywrightWorker` 类（子进程内实现）；`get_worker()` 在主进程返回 `PlaywrightWorkerProxy` 实例，在子进程返回真正实现。用 `CAMPUS_AUTH_ROLE` 环境变量区分进程角色。`_handle_login` 改为在子进程内创建本地 `threading.Event` 传给 LoginAttemptHandler（不再从 data 接收 cancel_event）。`get_worker()` 重启逻辑中删除 `cleanup_orphan_browsers()` 调用（移到子进程） |
...
| `app/container.py` | `_get_worker()` 仍返回 `get_worker()`；新增将 `engine` 引用注入 worker proxy（用于 `can_shutdown_worker`）；`startup()` 中删除 `cleanup_orphan_browsers()` 调用（移到子进程） |
