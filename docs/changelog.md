...
- 前端 `success_condition` 说明补充：单纯的校园网登录任务可以不设，系统会自动通过网络检测判断登录是否成功

### 新增功能

- **goto 步骤类型**：支持在执行过程中显式跳转到指定 URL，与顶层 `url` 自动导航互补
- **success_condition 成功条件简化**：复用 `eval` 步骤 + `store_as` 机制，顶层声明变量名即可自定义成功判定，替代上一版 `success_checks`/`failure_checks` 列表方案
