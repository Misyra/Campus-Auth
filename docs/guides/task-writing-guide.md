...
2. 模板变量 + 自定义变量（认证地址、账号密码、运营商等运行时配置 + 用户在 Web 控制台设置的自定义变量）
...
系统统一使用网络连通性检测判断任务成功与否：任务步骤全部完成后，自动检测网络是否可达（超时约 10 秒）。网络通 = 认证成功，网络断 = 认证失败。检测失败时会在日志中显示具体原因。
...
1. 变量名是否正确（区分大小写）
2. 模板语法是否正确（使用双大括号 `{{}}`）
3. 变量来源是否正确：系统变量（账号、密码等）自动可用；自定义变量需在任务 JSON 的 `variables` 字段中定义
