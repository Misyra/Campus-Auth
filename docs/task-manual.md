...
- 对配置的探测目标发起 TCP/HTTP/Portal 并发检测（`check_network_status()`）
  - TCP 探测：默认 `8.8.8.8:53`、`114.114.114.114:53`、`www.baidu.com:443`
  - HTTP 探测：HTTP/HTTPS GET 请求，仅 2xx 视为成功
  - Portal 探测：Captive Portal 检测（Apple/Microsoft/Firefox）
...
| `LOGIN_URL` | `http://172.29.0.2` | 认证页面地址（校园网网关默认地址） |
| `ISP` | 空 | 运营商关键字 |
