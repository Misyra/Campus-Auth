...
- **不要给 get_profile API 做密码掩码** — 本地单用户，`ENC:` 密文需要密钥文件才能解密，密钥与配置已分离存储；与 list_profiles 的策略差异不影响安全性
- **不要给截图文件名做路径穿越防护** — `TASK_ID_PATTERN ^[A-Za-z][A-Za-z0-9_]*$` 已排除路径穿越字符，当前安全；单用户桌面应用场景
- **不要从 template_vars 中移除明文密码** — 登录流程需要密码传递到 Worker，本地桌面应用无外部泄露面；已有异常脱敏机制
- **不要脱敏异常信息中的密码** — 本地单用户，详细错误帮助排查；与 repo_proxy 错误信息同理
- **不要脱敏初始化状态 API 的密码存在性** — 本地单用户日志，记录"密码=已设置"帮助排查启动问题
- **不要给日志做注入防护** — 本地单用户，日志文件在本地，用户自己配置的值；伪造日志无实际攻击面
- **不要给 repo_proxy 加 SSRF 防护** — 用户需手动输入恶意 URL，已限制 https?:// 协议；本地单用户

## 兼容性类 — 边缘场景，不处理

- **不要处理 Windows 网关检测的韩文/日文编码** — 韩文/日文 Windows 系统极少，校园网场景主要覆盖中文/英文系统
- **不要处理十六进制 SSID 误判** — 代码注释已承认，概率极低且无法完美解决；实际校园网 SSID 不会是纯十六进制字符串
