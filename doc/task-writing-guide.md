...
**自动降级（推荐）：** 执行器在普通 `fill()` 失败后会自动降级到强制输入模式（通过 JS 原生 setter 设置值并触发事件，模拟完整用户交互：focus → clear → set value → input → change → blur），无需额外配置。如果是 HK Posi 模式，建议在输入步骤前加一个 `click` 步骤先点击 tip 占位框以触发门户 JS 的状态切换。
