...
| `force` | 否 | `false` | 强制模式，跳过可见性检查。**通常无需手动设置**，执行器会在普通输入失败时自动降级到强制模式 |
...
**自动降级：** 即使不设置 `reveal_hidden`，执行器在普通 `fill()` 失败后也会自动降级到强制输入模式（通过 JS 原生 setter 设置值并触发事件），通常无需手动设置 `force: true`。

**手动 `force` 模式：** 仅在自动降级不适用的特殊场景下使用。启用 `force` 后会跳过可见性检查，直接通过 JavaScript 设置值。如果是 HK Posi 模式，建议额外加一个 `click` 步骤先点击 tip 占位框以触发门户 JS 的状态切换。
